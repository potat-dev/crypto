...
def encrypt(key, plaintext, mode=BlockMode.ECB, iv=None):
    ...
    match mode:
        case BlockMode.ECB:
            return vcc.encrypt_ecb(plaintext)
        case BlockMode.CBC:
            return vcc.encrypt_cbc(plaintext, iv)
        case BlockMode.PCBC:
            return vcc.encrypt_pcbc(plaintext, iv)
        case BlockMode.CFB:
            return vcc.encrypt_cfb(plaintext, iv)
        case BlockMode.OFB:
            return vcc.encrypt_ofb(plaintext, iv)
        case BlockMode.CTR:
            return vcc.encrypt_ctr(plaintext, iv)


def decrypt(key, ciphertext, mode=BlockMode.ECB, iv=None):
    if isinstance(key, str):
        key = key.encode('utf-8')
    ...
    if (iv is None) and (mode != BlockMode.ECB):
    ...
    match mode:
        case BlockMode.ECB:
            return vcc.decrypt_ecb(ciphertext)
        case BlockMode.CBC:
            return vcc.decrypt_cbc(ciphertext, iv)
        case BlockMode.PCBC:
            return vcc.decrypt_pcbc(ciphertext, iv)
        case BlockMode.CFB:
            return vcc.decrypt_cfb(ciphertext, iv)
        case BlockMode.OFB:
            return vcc.decrypt_ofb(ciphertext, iv)
        case BlockMode.CTR:
            return vcc.decrypt_ctr(ciphertext, iv)
...
def encrypt_bmp(key, in_filename, out_filename, mode=BlockMode.ECB, iv=None):
    with open(in_filename, 'rb') as input_file:
        bmp_header = input_file.read(BMP_HEADER)
        pixel_data = input_file.read()

    pixel_data = encrypt(key, pixel_data, mode=mode, iv=iv)
...
def decrypt_bmp(key, in_filename, out_filename, mode=BlockMode.ECB, iv=None):
    with open(in_filename, 'rb') as input_file:
        bmp_header = input_file.read(BMP_HEADER)
        pixel_data = input_file.read()

    pixel_data = decrypt(key, pixel_data, mode=mode, iv=iv)
