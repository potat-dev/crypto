...
class AES:
    ...
    def encrypt_ecb(self, plaintext):
        """
        Encrypts `plaintext` in ECB mode and PKCS
        """
    ...
    def decrypt_ecb(self, ciphertext):
        """
        Decrypts `ciphertext` in ECB mode and PKCS
        """
        blocks = []
...
def encrypt(key, plaintext, workload=100000, mode=BlockMode.ECB):
    ...
    match mode:
        ...
        case BlockMode.PCBC:
            ciphertext = aes.encrypt_pcbc(plaintext, iv)
        case BlockMode.CFB:
...
def decrypt(key, ciphertext, workload=100000, mode=BlockMode.ECB):
    ...
    match mode:
        case BlockMode.ECB:
            text = aes.decrypt_ecb(ciphertext)
        case BlockMode.CBC:
            text = aes.decrypt_cbc(ciphertext, iv)
        case BlockMode.PCBC:
            text = aes.decrypt_pcbc(ciphertext, iv)
        case BlockMode.CFB:
            text = aes.decrypt_cfb(ciphertext, iv)
        case BlockMode.OFB:
            text = aes.decrypt_ofb(ciphertext, iv)
        case BlockMode.CTR:
            text = aes.decrypt_ctr(ciphertext, iv)
